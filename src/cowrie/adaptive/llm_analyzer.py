...
class LLMAnalyzer:
    ...
    def retrieve_context(self, commands, labels, mitre_db, top_k=3):
        ...
        for t in mitre_db:
            ...
            for cmd in commands:
                if cmd in tool_boosts and tech_id in tool_boosts[cmd]:
                    score += 100 # massive boost for direct tool match
...
"""
        ...
        if result["intent"] == "Unknown" or len(result["intent"]) > 50 or hallucination_detected:
            if any("Malware Download" in l for l in rule_labels):
                result["intent"] = "Malware Delivery"
            elif any("Privilege Escalation" in l for l in rule_labels):
                result["intent"] = "Privilege Escalation"
            elif any("Brute Force" in l for l in rule_labels):
                result["intent"] = "Brute Force"
            elif any("Database Enumeration" in l for l in rule_labels):
                result["intent"] = "Database Enumeration"
            elif any("Network" in l for l in rule_labels) or any("Reconnaissance" in l for l in rule_labels):
                result["intent"] = "Reconnaissance"
        ...
        if result["attack_stage"] == "Unknown" or len(result["attack_stage"]) > 50 or hallucination_detected:
            if any("Privilege Escalation" in l for l in rule_labels):
                result["attack_stage"] = "Privilege Escalation"
            elif any("Malware Download" in l for l in rule_labels):
        ...
        # (Optional) Final check: Ensure MITRE technique is in summary if possible, but keep narrative flow
        # if mitre_info and mitre_info not in result["summary"] and primary and primary['id'] not in result["summary"]:
        #    result["summary"] += mitre_info
